...
Moreover, it is apparent that this terminology and the underlying concepts lead to miscommunication within operational forecasting.  Operational centres like the Bureau of Meteorology increasingly bring these conventional tidal concepts into the same organisational setting as geophysically `concrete' simulations (introduced in section \ref{sec:concrete}); and the subsequent potential for confusion is what motivates the following exposition.
\newline{}
...
Within operational authorities, \emph{tide prediction} is practically synonymous with the application of conventional harmonic methods. And it is here that the role of special frequencies (section \ref{sec:temporal}) has come to play a primary role.
...
Harmonic analysis has an especially long historical legacy that provides unavoidable concepts and jargon \citep{Cartwright:2000tt}\citep{Parker:2007wq}.
Recall that the basis for this analysis is not harmonic in the sense of an orthogonal Fourier series, but rather that it follows from the harmonic development of the \ATGP{} as described in section \ref{sec:basic_potential}. 
...
At its foundation, harmonic tidal analysis and prediction characterises the ocean response as a LTI system via a finite set of amplitude and phase terms that are identified \emph{empirically} by the application of timeseries methods.   This concept is stated a little too simply in the Australian Tide Manual as follows: ``\textit{ the purpose of tidal analysis is to represent the water level or current time series by a set of harmonics, or sine waves, each of them having a specific amplitude and phase}''\citep{PCTMSL-sp9}.
...
Equation \ref{eq:cos} shows the LTI model with constant amplitude $A_k$ and phase lag $g_k$; other terms are discussed below.  
%---------------------
\begin{equation}
...
In practice, conventional tide practice applies a range of elaborations and work-arounds in order to produce predictions.  Before discussing any further, the essential steps of the harmonic analysis and prediction process are  illustrated schematically in Figure \ref{fig:tidePractice} and summarised as follows:
\begin{enumerate}
    \item formulation of tidal constituent subset based on record length and other conventions;
    \item empirical projection of an observation timeseries onto a discrete set of non-orthogonal basis functions;
    \item application of conventions accounting for unresolved frequencies; 
    \item synthesis of tidal timeseries for any time period using matching constituent and other conventions.
\end{enumerate}.


The practical value of harmonic methods reflects the special significance and predictability of sea level variation occurring at tidal frequencies.  This value does not rely on explicitly describing ocean dynamics. Rather, the very \emph{absence} of fluid dynamics is a key strength of harmonic methods. 
...
Harmonic analysis can be formulated as an overdetermined inverse problem:
%--------------------
\begin{equation}
    \label{E:Axb}   
    \M{A} \V{x}=\V{b} 
\end{equation}
%--------------------
Where $\M{A}$ is a matrix of tidal timeseries basis functions, $\V{x}$ is the analysed tidal solution and $\V{b}$ is an observational record. The tidal basis functions are developed from the combination of the harmonic decomposition of $\eta_{eq}$ and a series of conventions.  The resulting basis is neither orthogonal nor complete.  The core assumption of stationarity for the finite series of basis functions is equivalent to an assumption that the basis has infinite span in time.

Practical applications of the harmonic formalism has largely evolved prior to modern cheap computing, and can appear somewhat baroque in isolation.
