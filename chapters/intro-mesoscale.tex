...
\BL{} is a non-tidal system in that \ATGP{} forcing is not applied, and in contrast to localised coastal simulations, there are no lateral open boundary conditions. Not only does the simulation exclude the gravitational tidal body forcing, it also excludes the surface boundary application of atmospheric pressure.   Surface pressure is excluded for analogous time and length scale reasons as the tides; and for the barotropic mode the pressure and tidal forcing can be formulated in an identical manner as surface gradients REF GILL.
